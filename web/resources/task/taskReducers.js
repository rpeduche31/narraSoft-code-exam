 ...
 *
 * NOTE: this is never called directly. Only by parent 'task' reducer (defined
...
function taskList(state = {
   ...
   * then recursively return an _additional_ taskList reducer.
   *
   * Otherwise, return the actual task lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional taskList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: taskList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_TASK_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_TASK_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_TASK_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
...
/**
 * Primary task reducer -
 *
 * This is the single source of truth for all things 'task' related within the
...
function task(state = {
  ...
  /**
   * "byId" is an object map of all task items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a task, the single task
  ...
  /**
   * "lists" corresponds to individual instances of the taskList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_TASK: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUESTTASKSCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVETASKSCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     * SINGLE TASK ACTIONS
     */
    case Actions.REQUEST_SINGLE_TASK: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_TASK_TO_MAP: {
    ...
    case Actions.REQUEST_CREATE_TASK: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_TASK: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.RECEIVE_DELETE_TASK: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_TASK: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_TASK_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: taskList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
